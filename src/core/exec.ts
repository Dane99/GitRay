...
/**
 * Environment tweaks every git call runs with.
 *
 * `GIT_TERMINAL_PROMPT` stops a credential helper from blocking on a prompt nobody can
 * see. `GIT_OPTIONAL_LOCKS=0` keeps read-only commands from taking the index lock, which
